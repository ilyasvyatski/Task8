...
class Calculator {
    func prepareValidOutput(_ expression: String, _ input: String) -> String {
        if expression.isEmpty && Character(input).isMathCharacter {
            return expression
        }
    ...
    func evaluate(_ expression: String) -> String
    {
        ...
        let result = expn.toFloat().expressionValue(with: nil, context: nil)
    ...
    func getPercentage(_ expression: String) -> String {
        ...
        guard let result = Double(lastNumber) else {
            return expression
        }
